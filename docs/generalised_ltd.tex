...
\end{lstlisting}
\texttt{surface\_origin="helper"} marks factors introduced by interpolation,
known-factor reconstruction, finite-pole recursion, terminal sectors, or
lower-sector CFF minors.  \texttt{singularity="spurious"} is an algorithmic
marker for a helper factor that is not one of the original causal surfaces,
for example a factor containing \(M\), a non-unit OSE coefficient, or a
polynomial reconstruction factor.  Lower-sector CFF causal surfaces can have
\texttt{surface\_origin="helper"} but
\texttt{singularity="physical"}.  In pretty output the class tag adds
\texttt{\_h} for helper origin and \texttt{\_sp} for the spurious marker, while
parentheses still mean numerator-only.  Thus \texttt{e\_h} is a helper-origin
E-surface with physical threshold meaning and \texttt{(e\_h\_sp)} is a
numerator-only helper factor.  Pure CFF denominator trees are tested not to
contain \texttt{\_sp} surfaces.
