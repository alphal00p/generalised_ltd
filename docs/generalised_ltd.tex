...
second term is a pinched graph minor with edge \(e\) contracted, also handled by
CFF.  Consequently the pure CFF denominator remains \(E\)-surface only.  The
non-trivial part is to lift the CFF expression of the contracted graph minor
back to an orientation of the original graph.  The lift is completely
mechanical:
\begin{enumerate}[leftmargin=2em]
  \item Contract the pinched edge set \(P\) in the denominator graph and build
        a CFF representation of the lower graph minor \(\Graph/P\).  Contracting,
        rather than merely deleting, preserves the graph minor seen by the
        remaining denominators and avoids disconnected bookkeeping half-edges in
        repeated-channel lower sectors.  If the lower graph factorizes into
        independent loop-energy components, build one CFF factor per component
        and multiply the component representations.
  \item Every lower-sector CFF term fixes loop energies
        \(K_{\mathfrak t}^{0,\,\Graph/P}\) in the original LMB.  If
  ...
  \item For every original edge, including each deleted edge, compute the
        ...
        \begin{equation}
          ...
          =
          \rho_e\cdot K_{\mathfrak t}^{0,\,\Graph/P}
          +\eta_e\cdot p^0 .
...
unit lower-sector contact.  The same explicit unit leaf is inserted when one
denominator branch is a strict prefix of another.  Thus chains \((a)\) and
\((a,b)\) serialize to
\[
  \eta_a^{-1}+\eta_a^{-1}\eta_b^{-1},
\]
rather than losing the terminal \(\eta_a^{-1}\) contribution inside the longer
branch.
...
denominator and contracts the other \(j\) copies in the lower graph minor.  In
JSON it contributes:
\begin{itemize}[leftmargin=2em]
...
CFF expression of the graph minor with the whole repeated channel contracted.
This is still an \(E\)-surface-only denominator representation; the additional
factors are explicitly numerator-side objects.
...
The \texttt{edge\_q0} map is the authoritative black-box numerator map.  If a
numerator refers instead to \texttt{loops[i]} and the DOT graph has an explicit
LMB carrier edge whose label is the corresponding loop name, the evaluator
constructs that loop four-vector from the carrier edge: its energy is
\texttt{edge\_q0[carrier]} and its spatial momentum is the carrier edge routing.
The stored \texttt{loop\_q0} remains a diagnostic and a fallback for graphs
without such a carrier edge.  Generalised contact samples are not required to
integrate to a single global loop-energy assignment, so this convention is what
makes \texttt{loops[i][0]} and \texttt{edges[carrier][0]} equivalent in the
