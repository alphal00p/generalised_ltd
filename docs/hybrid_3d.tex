...
quadratic construction, including repeated-signature graphs.  If a contacted
edge belongs to a repeated group, the contact sector simply lowers that
repeated denominator power by one copy; the resulting lower graph is again
expanded by ordinary CFF minors before any equal-energy limit is taken.
...
The pure CFF builder deliberately rejects repeated-signature graphs only when a
cap is higher than quadratic.  An earlier implementation reused the bounded
hybrid confluent limit for the quadratic repeated corner.  That expression is a
valid hybrid repeated-pole representation, but it can contain \(H\)-surfaces in
denominator trees and is therefore not the promised \(E\)-surface-only pure CFF
construction.  The implemented quadratic path instead keeps every lower-sector
CFF numerator energy map edge by edge and combines equal repeated copies only
through CFF \(E\)-surface minors.
