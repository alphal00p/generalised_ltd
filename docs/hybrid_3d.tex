...
linear form; each \(\zeta\in N_{\mathfrak o}\) is such a known linear factor in
on-shell and external energies.  Thus the previously used symbol
\(\zeta(\vec k;p,m)\) means exactly a cached linear surface factor appearing in
the numerator, not an additional denominator.  If no repeated channel is
present, then
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item The numerator sample value of a deleted quadratic edge is one of
        ...
        black-box samples.  The original JSON orientation therefore assigns
        \(q_e^0=+\omega_e\), \(0\), or \(-\omega_e\) to the missing edge and
...
remaining bounded edges of the same graph or lower graph.  This is the right
formal direction, but it is not yet a complete pure-CFF algorithm.  Once a
cubic or higher contact has been taken, the lower graph carries known
polynomial factors
\[
...
finite-pole divisions.  In the troublesome sectors the denominator of the
lower contact graph is not sufficiently convergent by itself: the missing
piece is an infinity/contact completion of the lower polynomial sector.  This
is precisely what is seen in the one-loop box tests with bounds such as
\((d_0,d_1,d_2,d_3)=(2,1,0,3)\) or \((3,1,2,0)\).  The original integral is
UV convergent, but the naive recursive finite-pole CFF contact expression
differs from LTD by an order-one amount.

Consequently the implementation exposes only the pure-CFF higher-power cases
for which the finite-pole construction is presently guaranteed:
\begin{enumerate}[leftmargin=2em]
  ...
  \item one-loop graphs with exactly one cubic cap \(d_h=3\), all other caps
        affine \(d_e\le1\), and no repeated-signature edge carrying a
        higher-than-quadratic cap.
\end{enumerate}
In these cases every pure-CFF denominator tree is built only from
...
The following pure-CFF sectors are deliberately rejected:
\begin{itemize}[leftmargin=2em]
  \item mixed cubic/quadratic sectors, for example
        \((2,1,0,3)\), \((3,1,2,0)\), and permutations of those patterns;
  \item quartic-or-higher pure-CFF caps, even if only one edge carries the
        high cap and all spectators are affine;
  \item higher-than-quadratic caps on repeated-signature edges.
\end{itemize}
The reason is not that such representations cannot exist.  Rather, the current
JSON denominator grammar is finite-pole CFF plus numerator-side linear factors.
The missing lower-sector infinity/contact completion must first be derived and
then encoded without introducing denominator \(H\)-surfaces before those cases
can honestly be exposed as pure CFF.
...
kite, and iterated-bubble examples by comparing the hybrid value to the
split-mass LTD limiting proxy.  In the absence of repeated propagators, hybrid
collapses exactly to LTD for any accepted bound vector.
...
This cubic box example is also the boundary of the current pure-CFF
implementation.  If the other box edges are affine, for example
\((d_0,d_1,d_2,d_3)=(3,1,1,1)\), the formula above is exact and agrees with
LTD.  If one spectator is quadratic, for example \((3,1,2,0)\), the naive
second contact would require the infinity/contact completion described above.
The command therefore rejects that case instead of emitting a misleading
E-only JSON representation.  The same cubic cap is allowed on a non-repeated
edge of a graph that contains other repeated propagators, provided the repeated
edges remain at most affine in the numerator.
...
\texttt{cff} & one-loop graphs & one \(d_h=3\), all other \(d_e\le1\), denominator \(E\)-surfaces only \\
\bottomrule
\end{tabular}
\end{center}

The last \texttt{cff} row includes one-loop graphs that contain repeated
propagators, as long as the cubic cap is not placed on a repeated-signature
edge and all repeated-signature edges are affine or absent from the bound list.
The rejected \texttt{cff} sectors are explicit in the code and tests:
mixed cubic/quadratic caps, quartic-or-higher caps, and higher-than-quadratic
caps on repeated-signature edges.  The rejection is intentional.  Those sectors
need a lower-sector contact/infinity completion that has not yet been encoded
in the E-only pure-CFF JSON grammar.
...
construction rather than the older hybrid fallback; the timing table should be
