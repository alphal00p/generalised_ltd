...
\begin{enumerate}[leftmargin=2em]
  ...
  \item arbitrary UV-convergent higher caps, including caps placed on
        repeated-signature edges, whenever every lower contact sector can be
...
integral.  Therefore the same lower-contact completion applies when the
higher-power numerator cap is placed on one of several repeated-signature
copies.  The hybrid formula remains useful for the LTD-like repeated-channel
organization, but it is not needed to make the pure-CFF high-power lift
well-defined.
...
box sectors with quartic caps on repeated copies, and the two-loop
rank-deficient penta sectors; in every accepted case the denominator surface
set is \(E\)-only and the value agrees with LTD or, for repeated propagators,
with the hybrid value and the split-mass LTD limiting proxy.
...
expression, and the split-mass LTD reference.  The older
\texttt{--cff-energy-degree-bounds} option is retained only for CFF-only legacy
diagnostics.
...
\texttt{cff} & high-power edge caps & UV-convergent caps with lower-contact completion; denominator \(E\)-surfaces only \\
\bottomrule
\end{tabular}
\end{center}
...
edges, provided the loop-energy UV check succeeds and the lower-contact
completion can be decomposed into non-zero loop-energy matroid components.
