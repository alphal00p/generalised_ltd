...
\begin{enumerate}[leftmargin=2em]
  ...
  \item arbitrary UV-convergent caps on graphs for which no repeated-signature
        edge carries a cap above two and every lower contact sector can be
        decomposed into non-zero loop-energy matroid components.
\end{enumerate}
...
The remaining deliberate pure-CFF rejection is a cap above two on a
repeated-signature edge.  Those sectors combine the higher-power numerator lift
with true repeated-pole denominator residues.  They are handled by the hybrid
formula, whose repeated-channel finite-difference construction was designed
for that situation.
...
for all UV-convergent one-loop box monomial caps through the tested boundary
\(\sum_e d_e\le6\).  The same higher cap is allowed on a non-repeated edge of a
graph that contains other repeated propagators; caps above two on the repeated
signature itself are still routed to the hybrid representation.

\phantomsection
\subsection*{1.3.2.c What the First Rejected Contact Sector Contains}
\addcontentsline{toc}{subsection}{1.3.2.c What the First Rejected Contact Sector Contains}

This subsection records the diagnostic calculation behind the phrase
``lower-sector infinity/contact completion''.  The calculation is now encoded in
the pure-CFF builder, but it is useful because the first formerly rejected
sector is simple enough to analyze completely.
...
\begin{equation}
  ...
  \label{eq:first-rejected-bubble}
\end{equation}
This bubble is the first place where the naive recursion was incomplete.
...
\begin{align}
  ...
  \omega_0^2+(\delta_{20}+\delta_{30})q_0^0
    +\delta_{20}\delta_{30}.
  \label{eq:first-rejected-polynomial-division}
\end{align}
Therefore
\begin{equation}
  ...
  +
  \frac{1}{D_1}.
  \label{eq:first-rejected-tadpole}
\end{equation}
...
\[
  -\,\frac{1}{2\omega_1}
\]
for the sector in \eqref{eq:first-rejected-bubble}.  Direct numerical
inspection of the hidden experimental recursion confirms exactly this.  Before
the terminal tree is restored, the experimental expression differs from LTD by
\(+1/(2\omega_1)\).  For the same random kinematics used in the tests,
\[
...
\[
  ...
  \quad (3,1,2,0),
\]
and their permutations show the same pattern: the missing piece is a single
...
unit-node encoding, all UV-convergent one-loop box monomial caps through the
boundary \(\sum_e d_e\le6\) agree with LTD at arbitrary precision.
...
pure-CFF tests now include the one-loop box sectors above and the two-loop
rank-deficient penta sectors; in every accepted case the denominator surface
set is \(E\)-only and the value agrees with LTD.  The one class still rejected
in pure CFF is a cap above two on a repeated-signature edge, where the
denominator itself is a repeated pole and the hybrid repeated-channel formula
...
\texttt{cff} & non-repeated high-power edges & UV-convergent caps with lower-contact completion; denominator \(E\)-surfaces only \\
\bottomrule
\end{tabular}
\end{center}

The last \texttt{cff} row includes one-loop and multiloop sectors with mixed
cubic/quadratic and quartic-or-higher caps, provided the high-power cap is not
on a repeated-signature edge and the loop-energy UV check succeeds.  The
remaining \texttt{cff} rejection is explicit in the code and tests: a
higher-than-quadratic cap on a repeated-signature edge.  Such a term combines a
higher numerator-energy lift with a repeated-pole denominator residue and is
handled by \texttt{hybrid}.
