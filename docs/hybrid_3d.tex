...
\end{itemize}
For several higher-power edges, the construction is recursive.  Pick one
active edge, split its contribution into the pole remainder and the contact
sector, then repeat the same operation on the remaining bounded edges of the
same graph or lower graph.  The recursion terminates when every remaining edge
is affine.  At that point the denominator is evaluated by ordinary CFF or,
for a hybrid repeated sector, by the repeated-channel master formula above.

In multiloop lower sectors the deleted graph may decompose into independent
loop-energy components.  The implementation uses the vector matroid of the
remaining loop-energy rows \(\rho_e\): two lower-sector edges are placed in the
same component when their rows are connected through a common rank basis.  A
CFF representation is generated for each full-rank component.  The Cartesian
product of component orientations fixes enough lower-sector edge energies to
solve the original LMB.  All component surfaces are then remapped to original
edge ids and multiplied.  The lifted deleted-edge energy
\(\widehat q_e^0\) can be a signed sum or difference of several on-shell
energies; when it is a difference, the numerator-side cached factor is an
\(H\)-surface.  This is harmless because it appears only in the numerator.
Pure CFF denominator trees remain \(E\)-surface only throughout the recursive
quadratic and one-loop higher-power constructions for non-repeated graphs, and
for split-mass repeated graphs where the repeated propagators have been made
distinct before the limiting test.

Hybrid mode uses the same bounded-degree algebra.  If the high-power edge is
not repeated, the lower-sector lift is exactly the one just described.  If the
high-power edge is repeated, the finite-pole samples \(a_j\omega_e\) are folded
into the repeated physical sample set and the remaining denominator derivatives
are generated by \eqref{eq:denominator-derivative-coeff}.  The numerator still
appears only through explicit sample values.

The pure CFF builder deliberately rejects unsplit repeated-propagator graphs
with non-affine energy caps.  An earlier implementation reused the bounded
hybrid confluent limit in this corner.  That expression is a valid hybrid
repeated-pole representation, but it can contain \(H\)-surfaces in denominator
trees and is therefore not the promised \(E\)-surface-only pure CFF
construction.  The missing pure CFF step is the equal-mass repeated-sector limit
of the contact CFF minors before any hybrid denominator-derivative organization
is introduced.
...
the pure CFF E-only repeated-sector contact limit is not yet implemented, so
the CLI rejects that row instead of displaying the older hybrid fallback as CFF.
The bounded hybrid reduction has 115 maps for this benchmark.
