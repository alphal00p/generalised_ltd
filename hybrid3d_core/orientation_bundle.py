...
class LinearEnergyExpr:
    ...
    def render(self) -> str:
        ...
        for i,c in self.internal_terms: parts.append((f'OSE[{i}]', c<0))
        for i,c in self.external_terms: parts.append((f'E[{i}]', c<0))
...
class SurfaceCacheBuilder:
    ...
    def intern(self, kind: str, expr: LinearEnergyExpr, label: str) -> int:
        expr=expr.canonical(); kind=classify_surface_kind(expr) if kind=='auto' else kind; key=(kind,expr.render())
...
def build_hybrid_bundle_raw(parsed):
    ...
    for branch,term in enumerate(reversed(cff.terms)):
        meta=dict(term.meta)
        meta.update({'source':'hybrid_repeated_cff_contraction_kernel','repeated_groups':[list(g.edge_ids) for g in rep_groups]})
        hybrid_terms.append(OrientationTerm(
            term.orientation_id + '|cff',
            'hybrid',
            branch,
